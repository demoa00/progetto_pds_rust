pub mod modified_flex {
    ...
    pub struct FlexMod<T> {
        ...
        visible: bool,
        visibility_condition: Option<Box<dyn Fn(&T) -> bool>>,
    }
    ...
    impl FlexParams {
        ...
        fn new(flex: f64, alignment: impl Into<Option<CrossAxisAlignment>>) -> Self {
            if flex <= 0.0 {
    ...
    impl<T: Data> FlexMod<T> {
        /// Create a new Flex oriented along the provided axis.
        pub fn for_axis(axis: Axis, visible: bool) -> Self {
            FlexMod {
                direction: axis,
                children: Vec::new(),
        ...
        ///
        pub fn row(start_visibility: bool) -> Self {
            Self::for_axis(Axis::Horizontal, start_visibility)
        }

        /// Create a new vertical stack.
        ///
        /// The child widgets are laid out vertically, from top to bottom.
        pub fn column(start_visibility: bool) -> Self {
            Self::for_axis(Axis::Vertical, start_visibility)
        }
        ...
        pub fn visible_if(mut self, f: impl Fn(&T) -> bool + 'static) -> Self {
            self.visibility_condition = Some(Box::new(f));
            self
        }
        ...
        pub fn add_default_spacer(&mut self) {
            let key = match self.direction {
                Axis::Vertical => druid::theme::WIDGET_PADDING_VERTICAL,
                Axis::Horizontal => druid::theme::WIDGET_PADDING_HORIZONTAL,
            };
            self.add_spacer(key);
        }
    ...
    impl<T: Data> Widget<T> for FlexMod<T> {
        ...
        fn update(&mut self, ctx: &mut UpdateCtx, _old_data: &T, data: &T, env: &Env) {
            match &self.visibility_condition {
                Some(condition) => {
                    if (condition)(data) != self.visible {
                        self.visible = !self.visible;
                        ctx.request_layout();
                        ctx.request_paint();
                    }
                }
                None => self.set_visible(true),
            }
                ...
                match child {
                    Child::Flex {
                        ...
                        let child_size = if any_changed || widget.layout_requested() {
                            ...
                            let old_size = widget.layout_rect().size();
                            let child_bc =
                                self.direction.constraints(&loosened_bc, 0.0, actual_major);
                            let child_size = widget.layout(ctx, &child_bc, data, env);
            ...
            for child in &mut self.children {
                match child {
                    ...
                    | Child::Flex {
                        ...
                        let child_pos: Point =
                            self.direction.pack(major, child_minor_offset).into();
            ...
            if self.direction != axis {
                ...
                for child in self.children.iter_mut() {
                    match child {
                        ...
                        Child::Flex { flex, .. } | Child::FlexedSpacer(flex, _) => {
                            total_flex += *flex
                        }
                    }
                }
